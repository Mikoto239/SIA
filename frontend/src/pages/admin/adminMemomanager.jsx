...
const AdminMemoManager = () => {
  ...
  const filteredMemoList = Icreatememos.filter((memo) =>
    memo.title.toLowerCase().includes(searchQueryMemoList.toLowerCase())
  );
  ...
  return (
    <>
        ...
        <div className="sidebar">
          <ul>
            ...
            <li className={activeMenuItem === 'Invite Members' ? 'active' : ''}>
              <Link to="/admin/inviteMember">Invite Members</Link>
            </li>
          </ul>
        </div>
      ...
      />
            <Link to="/admin/memo_create" className="link-to-send">Send Memo</Link>  
        ...
        <ul className="receivedMemoList">
          {filteredReceivedMemos.slice().reverse().map((memo) => (
            <li key={memo._id} className="receivedMemoItem">
              <Link to={`/admin/recieve_memo/${memo._id}`} onClick={(e) => handleRead(e, memo._id)}>
                <div className="memo-details">
                  <div className="memo-title">Title:{memo.title}</div>
